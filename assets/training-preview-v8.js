(() => {
  console.info("Math Problem Library: A4 preview PDF v8 loaded");
  ...
  function bracketEval(F,lo,hi){
    return `\\left[${F}\\right]_{${lo}}^{${hi}}`;
  }
  ...
  function genDef3(d){
    if(d===1){
      ...
      if(type==="exp"){
        ...
        const answer=`${expAt(hi)}-${expAt(lo)}`;
        return question(
          "次の定積分を求めよ。",
          `\\int_{${lo}}^{${hi}} e^x\\,dx`,
          answer,
      ...
      if(type==="cos"){
        const [lo,hi]=randomIntegerBounds(-9,9);
        const answer=`\\sin\\left(${hi}\\right)-\\sin\\left(${lo}\\right)`;
        return question(
          "次の定積分を求めよ。",
          `\\int_{${lo}}^{${hi}} \\cos x\\,dx`,
          answer,
    ...
    if(d===2){
      ...
      if(type==="exp"){
        const [lo,hi]=randomIntegerBounds(-9,9);
        const answer=`\\frac{${expAt(k*hi)}-${expAt(k*lo)}}{${k}}`;
        return question(
          "次の定積分を求めよ。",
          `\\int_{${lo}}^{${hi}} e^{${k}x}\\,dx`,
          answer,
          `di32exp:${k}:${lo}:${hi}`,
          {kind:"di32exp",k,lo,hi}
        );
      }

      if(type==="cos"){
        const [lo,hi]=randomIntegerBounds(-9,9);
        const answer=`\\frac{\\sin\\left(${k*hi}\\right)-\\sin\\left(${k*lo}\\right)}{${k}}`;
        return question(
          "次の定積分を求めよ。",
          `\\int_{${lo}}^{${hi}} \\cos ${k}x\\,dx`,
          answer,
          `di32cos:${k}:${lo}:${hi}`,
          {kind:"di32cos",k,lo,hi}
        );
      }

      // kx+c が区間内で 0 にならないように選ぶ。
      ...
      return question(
        ...
        answer,
        `di32log:${k}:${c}:${lo}:${hi}`,
        {kind:"di32log",k,c,lo,hi,left,right}
      );
    }
    ...
    if(d===3){
      ...
      if(type==="sub"){
        ...
        const [lo,hi]=randomIntegerBounds(-6,6,{avoidOpposites:true});
        const tLo=lo*lo+a;
        const tHi=hi*hi+a;
        ...
        return question(
          ...
          ratLatex(value),
          `di33sub:${a}:${n}:${lo}:${hi}`,
          {kind:"di33sub",a,n,lo,hi,tLo,tHi}
        );
      }

      if(type==="parts"){
        const [lo,hi]=randomIntegerBounds(-7,7);
        const answer=`\\left[\\left(x-1\\right)e^x\\right]_{${lo}}^{${hi}}`;
        return question(
          "次の定積分を求めよ。",
          `\\int_{${lo}}^{${hi}} xe^x\\,dx`,
          answer,
          `di33parts:${lo}:${hi}`,
          {kind:"di33parts",lo,hi}
        );
      }

      const [lo,hi]=randomIntegerBounds(-7,7);
      const answer=`\\left[x\\sin x+\\cos x\\right]_{${lo}}^{${hi}}`;
      return question(
        "次の定積分を求めよ。",
    ...
    if(d===4){
      ...
      if(type==="xexp"){
        const [lo,hi]=randomIntegerBounds(-7,7,{avoidOpposites:true});
        const answer=`\\frac{1}{2}\\left(${expAt(hi*hi)}-${expAt(lo*lo)}\\right)`;
        return question(
          "次の定積分を求めよ。",
      ...
      if(type==="log"){
        const a=randInt(1,5);
        const [lo,hi]=randomIntegerBounds(-8,8,{avoidOpposites:true});
        const vLo=lo*lo+a;
        const vHi=hi*hi+a;
        ...
        return question(
          ...
          answer,
          `di34log:${a}:${lo}:${hi}`,
          {kind:"di34log",a,lo,hi,vLo,vHi}
        );
      }

      if(type==="sin2"){
        const [lo,hi]=randomIntegerBounds(-9,9);
        const answer=`\\left[\\frac{x}{2}-\\frac{\\sin 2x}{4}\\right]_{${lo}}^{${hi}}`;
        return question(
          "次の定積分を求めよ。",
      ...
      // log x は定義域のため正の一桁整数のみ。
      const [lo,hi]=randomIntegerBounds(1,9,{positiveOnly:true});
      const answer=`\\left[x\\log x-x\\right]_{${lo}}^{${hi}}`;
      return question(
        "次の定積分を求めよ。",
        `\\int_{${lo}}^{${hi}} \\log x\\,dx`,
        answer,
    ...
    if(type==="x2exp"){
      const answer=`\\left[${cText}e^x\\left(x^2-2x+2\\right)\\right]_{${lo}}^{${hi}}`;
      return question(
        "次の定積分を求めよ。",
        `\\int_{${lo}}^{${hi}} ${cText}x^2e^x\\,dx`,
        answer,
        `di35x2exp:${c}:${lo}:${hi}`,
        {kind:"di35x2exp",c,lo,hi}
      );
    }

    if(type==="expsin"){
      const answer=`\\left[\\frac{${c}e^x}{2}\\left(\\sin x-\\cos x\\right)\\right]_{${lo}}^{${hi}}`;
      return question(
        "次の定積分を求めよ。",
        `\\int_{${lo}}^{${hi}} ${cText}e^x\\sin x\\,dx`,
        answer,
        `di35expsin:${c}:${lo}:${hi}`,
        {kind:"di35expsin",c,lo,hi}
      );
    }
    ...
    if(type==="expcos"){
      ...
      return question(
        ...
        answer,
        `di35expcos:${c}:${lo}:${hi}`,
        {kind:"di35expcos",c,lo,hi}
      );
    }

    const answer=`\\left[${cText}\\left(x^2\\sin x+2x\\cos x-2\\sin x\\right)\\right]_{${lo}}^{${hi}}`;
    return question(
      "次の定積分を求めよ。",
      `\\int_{${lo}}^{${hi}} ${cText}x^2\\cos x\\,dx`,
      answer,
      `di35x2cos:${c}:${lo}:${hi}`,
      {kind:"di35x2cos",c,lo,hi}
    );
  }
...
\\end{aligned}`;
    ...
    if(kind==="di32exp"){
      ...
      return `\\begin{aligned}
${item.expr}
&=\\left[\\frac{1}{${k}}e^{${k}x}\\right]_{${lo}}^{${hi}}\\\\
&=${ans}
\\end{aligned}`;
    }
    ...
    if(kind==="di32cos"){
      ...
      return `\\begin{aligned}
${item.expr}
&=\\left[\\frac{1}{${k}}\\sin ${k}x\\right]_{${lo}}^{${hi}}\\\\
&=${ans}
\\end{aligned}`;
    }

    if(kind==="di32log"){
      const {k,c,lo,hi}=data;
      return `\\begin{aligned}
${item.expr}
&=\\left[\\frac{1}{${k}}\\log|${k}x+${c}|\\right]_{${lo}}^{${hi}}\\\\
&=${ans}
\\end{aligned}`;
    }

    if(kind==="di33sub"){
      const {a,n,lo,hi,tLo,tHi}=data;
      return `\\begin{aligned}
t&=x^2+${a},\\quad dt=2x\\,dx\\\\
...
-\\int_{${lo}}^{${hi}}e^x\\,dx\\\\
&=\\left[\\left(x-1\\right)e^x\\right]_{${lo}}^{${hi}}\\\\
&=${ans}
\\end{aligned}`;
    }
...
t&=x^2,\\quad dt=2x\\,dx\\\\
${item.expr}
&=\\frac12\\int_{${lo*lo}}^{${hi*hi}}e^t\\,dt\\\\
&=\\frac12\\left[e^t\\right]_{${lo*lo}}^{${hi*hi}}\\\\
&=${ans}
\\end{aligned}`;
    }

    if(kind==="di34log"){
      const {a,lo,hi,vLo,vHi}=data;
      return `\\begin{aligned}
t&=x^2+${a},\\quad dt=2x\\,dx\\\\
x=${lo}&\\Rightarrow t=${vLo},\\quad x=${hi}\\Rightarrow t=${vHi}\\\\
${item.expr}
&=\\frac12\\int_{${vLo}}^{${vHi}}\\frac1t\\,dt\\\\
&=\\frac12\\left[\\log t\\right]_{${vLo}}^{${vHi}}\\\\
&=${ans}
\\end{aligned}`;
    }
...
\\end{aligned}`;
    ...
    if(kind==="di35x2exp"){
      ...
      return `\\begin{aligned}
\\int ${ct}x^2e^x\\,dx
&=${ct}e^x\\left(x^2-2x+2\\right)+C\\\\
${item.expr}
&=\\left[${ct}e^x\\left(x^2-2x+2\\right)\\right]_{${lo}}^{${hi}}\\\\
&=${ans}
\\end{aligned}`;
    }
    ...
    if(kind==="di35expsin"){
      ...
      return `\\begin{aligned}
\\int ${ct}e^x\\sin x\\,dx
&=\\frac{${c}e^x}{2}\\left(\\sin x-\\cos x\\right)+C\\\\
${item.expr}
&=\\left[\\frac{${c}e^x}{2}\\left(\\sin x-\\cos x\\right)\\right]_{${lo}}^{${hi}}\\\\
&=${ans}
\\end{aligned}`;
    }
...
\\end{aligned}`;
    ...
    if(kind==="di35x2cos"){
      ...
      return `\\begin{aligned}
\\int ${ct}x^2\\cos x\\,dx
&=${ct}\\left(x^2\\sin x+2x\\cos x-2\\sin x\\right)+C\\\\
${item.expr}
&=\\left[${ct}\\left(x^2\\sin x+2x\\cos x-2\\sin x\\right)\\right]_{${lo}}^{${hi}}\\\\
&=${ans}
\\end{aligned}`;
    }
