(() => {
  console.info("Math Problem Library: A4 preview PDF v5 loaded");
...
\\end{aligned}`;
    ...
    if(item.signature.startsWith("cs3b-")){
      const B=Number(sig[1]), C=Number(sig[2]);
      const h=new Rat(B,2);
      const h2=h.mul(h);
      const bText=B===1?"+ax":B===-1?"-ax":B>0?`+${B}ax`:`${B}ax`;
