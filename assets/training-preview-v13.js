(() => {
  console.info("Math Problem Library: A4 preview PDF v13 loaded");
  ...
  // 三角関数の定積分用：π/2 の整数倍を中心に出題する。
  // m は x = mπ/2 の m を表す。
  function randomHalfPiBounds(maxAbs=4){
    let a=0,b=0;
    do{
      a=randInt(-maxAbs,maxAbs);
      b=randInt(-maxAbs,maxAbs);
    }while(a===b);
    if(a>b) [a,b]=[b,a];
    return [a,b];
  }
  ...
  function sinHalfPi(m){
    return [0,1,0,-1][mod4(m)];
  }

  function cosHalfPi(m){
    return [1,0,-1,0][mod4(m)];
  }

  // rπ を教科書風に表示。
  function piRatLatex(r){
    r=toRat(r);
  ...
  function halfPiLatex(m){
    return piRatLatex(new Rat(m,2));
  }

  function scaledHalfPiLatex(m,k){
    // x = mπ/(2k)
    return piRatLatex(new Rat(m,2*k));
  }

  function piTermLatex(coeff,power=1){
    coeff=toRat(coeff);
    if(coeff.isZero()) return "";

    const sign=coeff.n<0 ? "-" : "";
    const a=coeff.abs();
    const p=power===1 ? "\\pi" : `\\pi^{${power}}`;

    if(a.n===a.d) return `${sign}${p}`;
    if(a.d===1) return `${sign}${a.n}${p}`;
    if(a.n===1) return `${sign}\\frac{${p}}{${a.d}}`;
    return `${sign}\\frac{${a.n}${p}}{${a.d}}`;
  }

  function polynomialPiLatex(a2,a1,a0){
    a2=toRat(a2); a1=toRat(a1); a0=toRat(a0);
    const terms=[];

    function pushSigned(text,isNegative){
      if(!text) return;
      if(terms.length===0){
        terms.push(text);
      }else if(isNegative){
    ...
    if(!a2.isZero()){
      pushSigned(piTermLatex(a2,2),a2.n<0);
    }
    if(!a1.isZero()){
      pushSigned(piTermLatex(a1,1),a1.n<0);
    }
    if(!a0.isZero()){
      const t=String(Math.abs(a0.n/a0.d));
      pushSigned(t,a0.n<0);
    }
  ...
  function linearPiValueAtXCos(m){
    // F(x)=x sin x + cos x, x=mπ/2
    const s=sinHalfPi(m);
    const c=cosHalfPi(m);
    return {
      a1:new Rat(m*s,2),
      a0:new Rat(c),
      latex:polynomialPiLatex(new Rat(0),new Rat(m*s,2),new Rat(c))
    };
  }

  function linearPiValueAtSin2(m){
    // F(x)=x/2-sin2x/4.
    // x=mπ/2 なら sin(mπ)=0 なので F=mπ/4.
    return {
      a1:new Rat(m,4),
      a0:new Rat(0),
      latex:polynomialPiLatex(new Rat(0),new Rat(m,4),new Rat(0))
    };
  }

  function simplifySignedDifference(a,b){
    // a - b を、0 や二重負号を残さない形にする。
    if(a==="0" && b==="0") return "0";
    if(b==="0") return a;
    if(a==="0"){
  ...
  function positiveRatioLatex(num,den){
    const r=new Rat(num,den);
    if(r.d===1) return `${r.n}`;
    return `\\frac{${r.n}}{${r.d}}`;
  }

  function exactSquareRootInt(n){
    if(n<0) return null;
    const s=Math.round(Math.sqrt(n));
    return s*s===n ? s : null;
  }

  function halfLogRatioSimplified(num,den){
    // 1/2 log(num/den) を、約分したうえで
    // 真数が有理数の平方なら log(sqrt(num/den)) まで整理する。
    const r=new Rat(num,den);
    const sn=exactSquareRootInt(r.n);
    const sd=exactSquareRootInt(r.d);
    ...
    if(sn!==null && sd!==null){
      ...
      return `\\log\\left(\\frac{${sn}}{${sd}}\\right)`;
    }

    const ratio=positiveRatioLatex(r.n,r.d);
    return `\\frac12\\log\\left(${ratio}\\right)`;
  }
  ...
  function expCoeffTerm(coeff, exponent){
    if(coeff===0) return "0";
    if(exponent===0) return String(coeff);
  ...
  function genDef3(d){
    if(d===1){
      ...
      if(type==="cos"){
        const [loM,hiM]=randomHalfPiBounds(4);
        const lo=halfPiLatex(loM), hi=halfPiLatex(hiM);
        const sLo=sinHalfPi(loM), sHi=sinHalfPi(hiM);
        const fHi=String(sHi), fLo=String(sLo);
        const answer=String(sHi-sLo);
        return question(
          "次の定積分を求めよ。",
          `\\int_{${lo}}^{${hi}} \\cos x\\,dx`,
          answer,
          `di31cospi:${loM}:${hiM}`,
          {kind:"di31cospi",loM,hiM,lo,hi,sLo,sHi,fHi,fLo}
        );
      }
    ...
    if(d===2){
      ...
      if(type==="cos"){
        const [loM,hiM]=randomHalfPiBounds(4);
        const lo=scaledHalfPiLatex(loM,k);
        const hi=scaledHalfPiLatex(hiM,k);
        const argLo=halfPiLatex(loM);
        const argHi=halfPiLatex(hiM);
        const sLo=sinHalfPi(loM), sHi=sinHalfPi(hiM);
        const fHi=ratLatex(new Rat(sHi,k));
        const fLo=ratLatex(new Rat(sLo,k));
        const answer=ratLatex(new Rat(sHi-sLo,k));
        ...
        return question(
          ...
          answer,
          `di32cospi:${k}:${loM}:${hiM}`,
          {kind:"di32cospi",k,loM,hiM,lo,hi,argLo,argHi,sLo,sHi,fHi,fLo}
        );
      }
    ...
    if(d===3){
      ...
      if(type==="parts"){
        ...
        const answer=simplifySignedDifference(fHi,fLo);
      ...
      const lo=halfPiLatex(loM), hi=halfPiLatex(hiM);
      const sLo=sinHalfPi(loM), sHi=sinHalfPi(hiM);
      const cLo=cosHalfPi(loM), cHi=cosHalfPi(hiM);
      const vLo=linearPiValueAtXCos(loM);
      const vHi=linearPiValueAtXCos(hiM);
      const ansA1=vHi.a1.sub(vLo.a1);
      const ansA0=vHi.a0.sub(vLo.a0);
      const answer=polynomialPiLatex(new Rat(0),ansA1,ansA0);
      ...
      return question(
        ...
        answer,
        `di33xcospi:${loM}:${hiM}`,
        {kind:"di33xcospi",loM,hiM,lo,hi,sLo,sHi,cLo,cHi,
         fLo:vLo.latex,fHi:vHi.latex,answer}
      );
    }
    ...
    if(d===4){
      ...
      if(type==="log"){
        ...
        // 真数を必ず約分し、平方比ならさらに整理。
        const ratio=new Rat(vHi,vLo);
        const ratioLatex=positiveRatioLatex(ratio.n,ratio.d);
        const combined=`\\frac12\\log\\left(${ratioLatex}\\right)`;
        const answer=halfLogRatioSimplified(vHi,vLo);
        ...
        return question(
          ...
          answer,
          `di34log:${a}:${lo}:${hi}`,
          {kind:"di34log",a,lo,hi,vLo,vHi,fHi,fLo,ratioLatex,combined}
        );
      }
      ...
      if(type==="sin2"){
        const [loM,hiM]=randomHalfPiBounds(4);
        const lo=halfPiLatex(loM), hi=halfPiLatex(hiM);
        const vLo=linearPiValueAtSin2(loM);
        const vHi=linearPiValueAtSin2(hiM);
        const ansA1=vHi.a1.sub(vLo.a1);
        const answer=polynomialPiLatex(new Rat(0),ansA1,new Rat(0));
        ...
        return question(
          ...
          answer,
          `di34sin2pi:${loM}:${hiM}`,
          {kind:"di34sin2pi",loM,hiM,lo,hi,fLo:vLo.latex,fHi:vHi.latex,answer}
        );
      }
...
\\int\\log x\\,dx
&=\\int (\\log x)(x)'\\,dx\\\\
&=x\\log x-\\int (\\log x)'x\\,dx\\\\
&=x\\log x-\\int\\frac1x\\,x\\,dx\\\\
&=x\\log x-\\int1\\,dx\\\\
&=x\\log x-x+C
\\end{aligned}`;
    }
...
&=-${ct}e^x\\cos x+\\int (${ct}e^x)'\\cos x\\,dx\\\\
&=-${ct}e^x\\cos x+\\int ${ct}e^x\\cos x\\,dx\\\\
\\cos x&=(\\sin x)'\\\\
\\int ${ct}e^x\\cos x\\,dx
&=${ct}e^x\\sin x-\\int (${ct}e^x)'\\sin x\\,dx\\\\
&=${ct}e^x\\sin x-I\\\\
...
\\int ${ct}x^2\\cos x\\,dx
&=\\int ${ct}x^2(\\sin x)'\\,dx\\\\
&=${ct}x^2\\sin x-\\int (${ct}x^2)'\\sin x\\,dx\\\\
...
\\end{aligned}`;
    ...
    if(kind==="di31cospi"){
      const {lo,hi,sLo,sHi}=data;
      const middle=`${sHi}-\\left(${sLo}\\right)`;
      return `\\begin{aligned}
${item.expr}
&=\\left[\\sin x\\right]_{${lo}}^{${hi}}\\\\
&=\\sin\\left(${hi}\\right)-\\sin\\left(${lo}\\right)\\\\
&=${middle}\\\\
&=${ans}
\\end{aligned}`;
    }
...
\\end{aligned}`;
    ...
    if(kind==="di32cospi"){
      const {k,lo,hi,argLo,argHi,sLo,sHi}=data;
      return `\\begin{aligned}
${item.expr}
&=\\left[\\frac1${k}\\sin ${k}x\\right]_{${lo}}^{${hi}}\\\\
&=\\frac1${k}\\left\\{\\sin\\left(${argHi}\\right)-\\sin\\left(${argLo}\\right)\\right\\}\\\\
&=\\frac1${k}\\left\\{${sHi}-\\left(${sLo}\\right)\\right\\}\\\\
&=${ans}
\\end{aligned}`;
    }
...
\\end{aligned}`;
    ...
    if(kind==="di33xcospi"){
      const {lo,hi,sLo,sHi,cLo,cHi,fHi,fLo}=data;
      return `\\begin{aligned}
\\cos x&=(\\sin x)'\\\\
${item.expr}
&=\\int_{${lo}}^{${hi}}x(\\sin x)'\\,dx\\\\
&=\\left[x\\sin x\\right]_{${lo}}^{${hi}}
-\\int_{${lo}}^{${hi}}(x)'\\sin x\\,dx\\\\
&=\\left[x\\sin x+\\cos x\\right]_{${lo}}^{${hi}}\\\\
&=\\left\\{${hi}\\cdot${sHi}+${cHi}\\right\\}
-\\left\\{${lo}\\cdot${sLo}+${cLo}\\right\\}\\\\
...
\\end{aligned}`;
    ...
    if(kind==="di34log"){
      const {a,lo,hi,vLo,vHi,fHi,fLo,ratioLatex,combined}=data;
      const needFinal=combined!==ans;
      return `\\begin{aligned}
t&=x^2+${a},\\quad dt=2x\\,dx\\\\
...
&=${fHi}-\\left(${fLo}\\right)\\\\
&=\\frac12\\log\\left(${ratioLatex}\\right)${needFinal?`\\\\
&=${ans}`:""}
\\end{aligned}`;
    }
...
1&=(x)'\\\\
${item.expr}
&=\\int_{${lo}}^{${hi}}(\\log x)(x)'\\,dx\\\\
&=\\left[x\\log x\\right]_{${lo}}^{${hi}}
-\\int_{${lo}}^{${hi}}(\\log x)'x\\,dx\\\\
...
&=\\int ${ct}x^2(e^x)'\\,dx\\\\
&=${ct}x^2e^x-2${ct}\\int xe^x\\,dx\\\\
\\int xe^x\\,dx&=xe^x-\\int (x)'e^x\\,dx=xe^x-e^x\\\\
\\therefore\\quad
\\int ${ct}x^2e^x\\,dx&=${ct}e^x(x^2-2x+2)+C\\\\
${item.expr}
...
\\therefore\\quad
\\int ${ct}x^2\\cos x\\,dx&=${ct}(x^2\\sin x+2x\\cos x-2\\sin x)+C\\\\
${item.expr}
