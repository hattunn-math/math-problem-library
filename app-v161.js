...
function renderTeachers(){
  ...
  ensureTeacherOrder();
  const filter=$("#teacherFilterDepartment")?.value||"";
  const names=sortedTeachers().filter(name=>!filter||profileOf(name).department===filter);
  $("#teacherCards").innerHTML=names.length?names.map(name=>teacherCardHtml(name)).join(""):`<p class="muted">該当する教員はいません。</p>`;
...
function bindTeacherCardEvents(){
  $$(".department-select").forEach(el=>el.onchange=()=>{
    ...
    const oldDep=profileOf(t).department;
    profileOf(t).department=el.value;
    if(state.teacherOrder?.[oldDep])state.teacherOrder[oldDep]=state.teacherOrder[oldDep].filter(x=>x!==t);
    if(el.value){
      state.teacherOrder[el.value]=state.teacherOrder[el.value]||[];
      if(!state.teacherOrder[el.value].includes(t))state.teacherOrder[el.value].push(t);
    }
    addHistory("教員設定",`${t}：担当教科 ${el.value||"未設定"}`);save();renderAll();
  });
  ...
  $$(".move-teacher-up").forEach(b=>b.onclick=()=>moveTeacherWithinDepartment(b.dataset.teacher,-1));
  $$(".move-teacher-down").forEach(b=>b.onclick=()=>moveTeacherWithinDepartment(b.dataset.teacher,1));
  $$(".delete-teacher").forEach(b=>b.onclick=()=>{
    ...
    Object.keys(state.teacherOrder||{}).forEach(dep=>state.teacherOrder[dep]=state.teacherOrder[dep].filter(x=>x!==t));
    delete state.teacherProfiles[t];save();renderAll();
  });
}
function moveTeacherWithinDepartment(name,delta){
  const dep=profileOf(name).department;
  if(!dep)return;
  ensureTeacherOrder();
  const arr=state.teacherOrder[dep]||[];
  const i=arr.indexOf(name),j=i+delta;
  if(i<0||j<0||j>=arr.length)return;
  [arr[i],arr[j]]=[arr[j],arr[i]];
  state.teacherOrder[dep]=arr;
  addHistory("教員表示順",`${dep}：${name}を${delta<0?"上":"下"}へ移動`);
  save();renderTeachers();renderDaily();
}
function addTeacher(){
  ...
  state.teacherOrder=state.teacherOrder||{};
  state.teacherOrder[department]=state.teacherOrder[department]||[];
  state.teacherOrder[department].push(name);
  $("#teacherName").value="";
...
function init(){
  ...
  $("#teacherFilterDepartment").innerHTML=`<option value="">すべての教科</option>`+DEPARTMENTS.map(d=>`<option value="${d}">${d}</option>`).join("");
